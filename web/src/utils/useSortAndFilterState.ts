...
export function useSortAndFilterState() {
    ...
    useEffect(() => {
        ...
        if (querySortDir === null && querySortField === null) {
            ...
            if (['asc', 'desc'].includes(querySortDir) && ['name', 'type', 'size'].includes(querySortField)) {
                setSortState({sortField: querySortField as keyof FileData, sortDir: querySortDir as SortDirection})
            }
        }
    ...
    const handleSearchChange = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        setSearch(val);
        setSearchParams({sort: sortState.sortField, dir: sortState.sortDir, filter: val})
      };
    ...
    const handleSortChange = (sortField: keyof FileData) => {
        ...
        setSearchParams({sort: newSortState.sortField, dir: newSortState.sortDir, filter: search})
    };
